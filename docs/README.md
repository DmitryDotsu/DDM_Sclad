...
Спроектировать и создать базу данных в PostgreeSQL (либо иной СУБД по выбору)
...
1)Не менее 6 таблиц
2)Ограничения: DEFAULT, CHECK, PRIMARY KEY, UNUIQUE, FOREIGN KEY
3)Индексы
4)Проекции (VIEW): по одной таблице, по нескольким таблицам, используя GROUP BY и HAVING, 
...
3)Как минимум одна форма должна обеспечивать ввод данных в две таблицы с отношением 1:М
...
Отчет оформляется в соответствии с методическими рекомендациями кафедры ПО.
